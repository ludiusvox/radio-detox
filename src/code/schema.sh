...
# Pick the symlink for the path; ideally, it's a path that is in your 
...
# The filename stem for the downloadable zip, as it exists on the Github 
filesystem,
...
# `schema-macos-opt.sh` is just the arbitrary name from which the symlink 
will be based
...
java -cp \$(echo \${SCHEMACRAWL_DIR}/lib/*.jar | tr ' ' 
':'):\${SCHEMACRAWL_DIR}/config schemacrawler.Main "\$@"
SCSCRIPT
